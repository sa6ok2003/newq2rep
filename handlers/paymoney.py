...
QIWI_TOKEN = '8614f515cfca67dd3b116baa369dcc0a' #Токен
QIWI_ACCOUNT = '+79198448283' #Номер киви
...
async def profile(call: types.callback_query):
    ...
    await bot.edit_message_text(chat_id=call.message.chat.id,message_id=call.message.message_id, text=
                    ...
                    '<pre>Для пополнения баланса другими способами или в ручном режиме, вы можете обратиться в поддержку:</pre>@QWsupport',reply_markup=markup, parse_mode='html')
...
async def proverka_balans(call: types.callback_query):
    ...
    if a == 1:
        channeg_money(call.message.chat.id,100) #ЗАМЕНА НА summa
